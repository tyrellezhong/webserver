...
void HttpRequest::ParsePost_() {
    if(method_ == "POST" && header_["Content-Type"] == "application/x-www-form-urlencoded") {
        ...
        if(DEFAULT_HTML_TAG.count(path_)) {
            int tag = DEFAULT_HTML_TAG.find(path_)->second; // 0 是注册，1 是登录
            LOG_DEBUG("Tag : %d", tag);
            if(tag == 0 || tag == 1) {
                bool isLogin = (tag == 1);
...
bool HttpRequest::UserVerify(const string &name, const string &pwd, bool isLogin) {
    ...
    if(!isLogin) { flag = true; } // flag 注册标志及返回状态
    /* 查询用户及密码 */
    ...
    while(MYSQL_ROW row = mysql_fetch_row(res)) {
        ...
        /* 登录行为，用户名密码是否匹配*/
        if(isLogin) {
            if(pwd == password) { flag = true; } // 登录成功
            else {
                flag = false;
                LOG_DEBUG("pwd error!");
            }
    ...
    // 释放数据库,SqlConnRAII析构以及释放过了，是不是不需要再释放了
